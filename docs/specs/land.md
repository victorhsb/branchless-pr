...
| Native integration enabled, local PRs are partially stacked (one native Stack plus an unstacked suffix) | return an actionable unsupported error before editing or merging a PR or mutating a branch; the error identifies the top PR URL for effective style `whole-stack` and the bottom PR URL for `bottom-only` |
