...
func TestFilterCommentItemsUnresolvedAuthorAndKind(t *testing.T) {
	...
	got := filterCommentItems(items, commentsOptions{unresolvedOnly: true}, kinds)
	if len(got) != 2 {
...
func TestWriteCommentsReportJSONIsSingleObject(t *testing.T) {
	...
	if err := writeCommentsReport(&out, report, "json"); err != nil {
		t.Fatal(err)
	}
...
func TestWriteCommentsReportTextCoversEmptyMissingAndWarnings(t *testing.T) {
	...
	var out bytes.Buffer
	if err := writeCommentsReport(&out, report, "text"); err != nil {
		t.Fatal(err)
	}
	text := out.String()
...
func TestBuildCommentsReportHandlesMissingEmptyFailureAndAuth(t *testing.T) {
	...
	report, err := buildCommentsReport(app, commentsOptions{}, kinds, func(prRef string) (*pr.PullRequestComments, error) {
	...
	_, err = buildCommentsReport(app, commentsOptions{}, kinds, func(string) (*pr.PullRequestComments, error) {
...
func TestBuildCommentsReportEmptyStackDoesNotFetch(t *testing.T) {
	...
	called := false
	report, err := buildCommentsReport(app, commentsOptions{}, kinds, func(string) (*pr.PullRequestComments, error) {
		called = true
		return nil, nil
	})
	if err != nil {
...
func TestRootCleanCheckExemptsComments(t *testing.T) {
	...
	if !strings.Contains(string(data), `cmd.Name() != "comments"`) {
		t.Fatal("root clean check does not exempt comments command")
	}
}
