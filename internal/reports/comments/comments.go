...
func RunWithFetcher(app *AppContext, opts Options, w io.Writer, fetch Fetcher) error {
	...
	report, err := Build(app, opts, kinds, fetch)
	if err != nil {
		return err
	}
	return Write(w, report, opts.Format)
}
...
func filterCommentItems(items []pr.CommentItem, opts Options, kinds map[string]bool) []pr.CommentItem {
...
func filterCommentItem(item pr.CommentItem, opts Options, kinds map[string]bool, ignored map[string]bool) (pr.CommentItem, bool) {
	if !kinds[item.Kind] {
