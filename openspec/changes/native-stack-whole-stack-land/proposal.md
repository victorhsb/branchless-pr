> **STATUS: BLOCKED (2026-07-28)** — The core mechanism (`gh pr merge --rebase --auto` on the tip PR) contradicts current gh-stack documentation: the FAQ states auto-merge is "currently unavailable for stacked PRs, for both direct merges and the merge queue". Paused pending decision: (1) probe empirically whether `--auto` works on a stacked PR in a preview-enabled repo (docs may lag), (2) redesign around direct `gh pr merge --rebase` (no `--auto`), or (3) keep the current refusal until GitHub ships auto-merge for stacks. See https://github.github.com/gh-stack/faq/#can-i-enable-auto-merge-on-a-stacked-pr
