...
The `stack-pr view` command SHALL support a `--format json` mode that produces machine-readable JSON instead of the default ANSI-colored text and, when native integration is enabled, includes nullable GitHub Stack membership fields.
...
- **AND** output contains no ANSI escape sequences, terminal hyperlinks, warnings, progress logs, or extra stdout text

#### Scenario: Missing PR fields
- **WHEN** a stack entry has no associated PR
- **THEN** `pr_url` is `""` and `pr_number` is `0`
- **AND** every `github_stack_*` field is `null`
